...
std::vector<Summary*> Evolution::solve(int t_test_num) {
	...
	for(int t = 1; t <= t_test_num; t++) {
		...
		for(int g = 1; g <= config::GEN; g++) {
			...
			for(int i = 0; i < m_population.size(); i++) {
				generation_summary->addResult(m_population[i]);
			}
...
void Evolution::createPopulation() {
	/*for (std::vector<Individual*>::iterator i = m_population.begin(); i != m_population.end(); ++i)
		delete* i;*/
	m_population.clear();
